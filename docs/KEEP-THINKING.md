...
### Iteration 9: Multi-Step Clinical Reasoning Pipeline and PMC Full-Text Context (Feb 11)
**What changed:** Added Plan-Execute-Verify reasoning pipeline and wired full PMC Open Access articles into the context window.
**Why:** Simple single-pass Q&A was insufficient for complex clinical questions — drug safety, dosage interactions, and symptom combinations require multi-step reasoning. A patient asking "Can I take ibuprofen with my beta-blocker?" needs the AI to: (1) identify relevant knowledge domains, (2) consult pharmacology guidelines and drug interaction data, (3) generate a safe response, and (4) verify accuracy against clinical evidence. Single-pass generation skips steps 1 and 4.
**Before → After:** Before: QaAssistant sent all questions through the same single-pass Opus call with extended thinking. PMC articles were truncated to 4,000 words. After: Complex clinical questions (detected by keyword triggers) route through ClinicalReasoningPipeline with 3 phases — Plan (identify knowledge domains), Execute (generate with full context + thinking), Verify (validate against guidelines). PMC articles now load up to 50,000 words (~66K tokens) each, demonstrating Opus 4.6's 1M context window. SSE streams phase indicators so the patient sees the AI's deliberation process. The entire system is documented in `docs/opus-4.6-usage.md` with per-subsystem thinking budgets, tier comparison table, and prompt caching economics.

### Iteration 10: True Progressive Streaming — SDK Bypass for Real-Time Token Delivery (Feb 12)
**What changed:** Replaced the Anthropic PHP SDK's streaming transport with raw curl for all streaming endpoints.
**Why:** The patient-facing chat showed the AI "thinking" for 7+ seconds, then dumped the entire response at once — destroying the conversational feel. In a medical context, this is particularly bad: a patient asking about medication side effects wants immediate reassurance that the system is working, not an anxiety-inducing blank screen. Progressive streaming (tokens appearing as they're generated) provides visual feedback that the AI is actively processing their question, creating a more trustworthy and human-like interaction.
**Before → After:** Before: `anthropic-ai/sdk` v0.5.0 used PSR-18 `sendRequest()` which buffered the entire response (~7s) before yielding any tokens. All 90+ chunks arrived simultaneously. After: `AnthropicClient::rawCurlStream()` uses `curl_multi_init()` + `CURLOPT_WRITEFUNCTION` to receive and yield tokens the instant they arrive from the API. Time to first token dropped from 7.2s to 1.8s. Thinking tokens stream progressively, then text tokens follow — the patient sees the AI "working" within 2 seconds. Non-streaming methods (SOAP generation, term extraction) still use the SDK normally.
