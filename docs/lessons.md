...
### Lesson 23: DemoSeeder must generate medical_terms for ALL visit notes, not just one
- **Bug:** Term highlighter showed no highlights on Heart Failure and Hypertension visits because DemoSeeder only hardcoded `medical_terms` for the PVC visit. HF/HTN visit notes had `medical_terms = NULL`.
- **Fix:** Added `extractMissingMedicalTerms()` to DemoSeeder that calls `TermExtractor` on any visit note without terms after seeding. Now all 3 demo visits get highlighted terms automatically.
- **Takeaway:** When a feature depends on data generated by another service (AI term extraction), ensure the seeder invokes that service for ALL seed data, not just the first scenario. Don't assume manually hardcoding works for every case — automate.
