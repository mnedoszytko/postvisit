  ...
  <DoctorLayout>
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 p-6 flex items-center gap-6">
        ...
        <div>
          <h1 class="text-2xl font-bold text-gray-900">{{ patientFullName }}</h1>
          <p class="text-gray-500">{{ patient?.age ? `Age ${patient.age}` : '' }} {{ patient?.conditions?.map(c => c.display_name || c.name)?.join(', ') || '' }}</p>
        </div>
      ...
      <section>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 divide-y divide-gray-100">
          ...
          <div v-for="visit in visits" :key="visit.id" class="p-4">
            ...
            <p class="text-sm text-gray-500">{{ formatDate(visit.started_at || visit.visit_date) }}</p>
          ...
          <div v-for="notif in notifications" :key="notif.id" class="border-b border-gray-100 last:border-b-0 p-4">
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <!-- Reply button (only for patient messages, not for doctor replies) -->
                <button
...
async function fetchNotifications() {
    ...
    try {
        ...
        // Filter notifications relevant to this patient's visits
