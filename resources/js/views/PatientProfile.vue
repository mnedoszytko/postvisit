...
const invitationMessage = computed(() => {
    ...
    if (!doc) return '';
    const context = doc.primary_specialty
      ? ` Based on your recent ${doc.primary_specialty} visit, I'd recommend we check in again soon.`
      : '';
    return `Please select a convenient date and time for your follow-up appointment.${context}`;
});
